	...
	try {
		...
		if (type === "daily") {
			if (kv) {
				...
				if (cached) {
					...
					const todayRows = await db
						.prepare("SELECT DATE(created_at) as date, COUNT(*) as count, COUNT(DISTINCT ip) as uv FROM pageviews WHERE is_crawler = 0 AND DATE(created_at) = ? GROUP BY DATE(created_at)")
						.bind(todayStr)
						.all<{ date: string; count: number; uv: number }>();
					result = [...historical, ...(todayRows.results ?? [])];
				} else {
					const rows = await db
						.prepare("SELECT DATE(created_at) as date, COUNT(*) as count, COUNT(DISTINCT ip) as uv FROM pageviews WHERE is_crawler = 0" + dateFilter + " GROUP BY DATE(created_at) ORDER BY date ASC")
						.bind(...dateBind)
						.all<{ date: string; count: number; uv: number }>();
					result = rows.results ?? [];
			...
			for (const r of merged) {
				...
				if (r.path === "/" || r.path === "/posts/" || r.path === "/post/") continue;
