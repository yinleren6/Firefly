...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    const dateFilter = days > 0 ? " AND created_at >= DATE('now', ? || ' days')" : "";
    const dateBind = days > 0 ? [`-${days}`] : [];

    if (type === "daily") {
      const rows = await db
        .prepare(
          "SELECT DATE(created_at) as date, COUNT(*) as count, COUNT(DISTINCT ip_hash) as uv FROM pageviews WHERE is_crawler = 0" + dateFilter + " GROUP BY DATE(created_at) ORDER BY date ASC"
        )
        .bind(...dateBind)
    ...
    if (type === "top") {
      const rows = await db
        .prepare(
          "SELECT path, COUNT(*) as count FROM pageviews WHERE is_crawler = 0" + dateFilter + " GROUP BY path ORDER BY count DESC LIMIT 50"
        )
        .bind(...dateBind)
      ...
      for (const r of rows.results ?? []) {
        if (r.path === "/posts/{canonicalSlug}/" || r.path.includes("{canonicalSlug}")) continue;
      ...
      if (otherCount > 0) result.push({ path: "/其他页面/", count: otherCount });
    ...
    if (type === "referrer") {
      const rows = await db
        .prepare(
          "SELECT referrer, COUNT(*) as count FROM pageviews WHERE is_crawler = 0 AND referrer != ''" + dateFilter + " GROUP BY referrer ORDER BY count DESC LIMIT 50"
        )
        .bind(...dateBind)
